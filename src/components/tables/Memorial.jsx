		...
		<div className='flex flex-col gap-5 p-5'>
			...
			<div className='flex items-center gap-4'>
				...
				<div className='flex flex-col gap-1'>
					<span className='font-mono text-[15px] tracking-wide text-fontz/70'>
						{entry.operator?.callSign || "Unknown"}
					</span>
					<span className='font-mono text-[11px] tracking-widest uppercase text-red-500/80'>
			...
			<button
				...
				className='w-full flex items-center justify-center gap-2 font-mono text-[11px] tracking-widest uppercase px-3 py-2.5 rounded border border-amber-800/30 bg-amber-500/10 hover:bg-amber-400 text-amber-500 hover:text-blk hover:border-amber-400 transition-all'>
				<FontAwesomeIcon icon={faBoltLightning} className='text-[11px]' />
				Revive Operator
			</button>
		</div>
	);
	...
	if (KIAOperators.length === 0) {
		return (
			<div className='flex items-center justify-center py-6'>
				<span className='font-mono text-[11px] tracking-widest text-lines/20 uppercase'>No Casualties</span>
			</div>
		);
	}
			...
			{KIAOperators.map((entry, index) => {
				...
				return (
					<div
						...
						{/* Avatar */}
						<div className='relative shrink-0'>
							<div className='w-9 h-9 rounded-full border border-red-900/40 overflow-hidden bg-highlight'>
								<img
									className='w-full h-full object-cover object-top grayscale opacity-60'
									src={entry.operator?.imageKey || entry.operator?.image || "/ghost/Default.png"}
									alt={entry.operator?.callSign || "Operator"}
							...
							<span className='absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border border-blk bg-red-500' />
						</div>

						{/* Info */}
						<div className='flex-1 min-w-0 flex flex-col gap-0.5'>
							<div className='flex items-center justify-between gap-2'>
								<span className='font-mono text-[12px] text-fontz/60 truncate'>
									{entry.operator?.callSign || "Unknown"}
								</span>
								<span className='font-mono text-[10px] tracking-widest uppercase text-red-500/70 shrink-0'>
									KIA
								</span>
							</div>
							<span className='font-mono text-[10px] text-lines/25 truncate'>
