...
// ─── Injury detail sheet ──────────────────────────────────────
function InjuryDetail({ entry, onDischarge, onBack }) {
	...
	return (
		<div className='flex flex-col gap-5 p-5'>
			...
			<button
				onClick={onBack}
				className='flex items-center gap-2 font-mono text-[11px] tracking-widest uppercase text-lines/40 hover:text-fontz transition-colors self-start'>
			...
			<div className='flex items-center gap-4'>
				...
				<div className='flex flex-col gap-1'>
					<span className='font-mono text-[15px] tracking-wide text-fontz'>
						{entry.operator?.callSign || "Unknown"}
					</span>
					<span className='font-mono text-[11px] tracking-widest uppercase text-amber-400'>
						WIA — Wounded In Action
					</span>
					<span className='font-mono text-[11px] text-lines/40'>
			...
			<div className='flex flex-col gap-2'>
				<div className='flex items-center justify-between'>
					<span className='font-mono text-[11px] tracking-widest uppercase text-lines/35'>
						Recovery Progress
					</span>
					<span className='font-mono text-[11px] tabular-nums text-lines/40'>
				...
				<span className='font-mono text-[11px] text-lines/30 text-right'>{progress}%</span>
			</div>

			{/* Discharge */}
			<button
				onClick={handleDischarge}
				className='w-full flex items-center justify-center gap-2 font-mono text-[11px] tracking-widest uppercase px-3 py-2.5 rounded border border-btn/30 bg-btn/10 hover:bg-btn text-btn hover:text-blk hover:border-btn transition-all'>
				<FontAwesomeIcon icon={faSyringe} className='text-[11px]' />
				Discharge Operator
			</button>
		</div>
	);
...
function Row({ label, value }) {
	return (
		<div className='flex items-center justify-between'>
			<span className='font-mono text-[10px] tracking-widest uppercase text-lines/30'>{label}</span>
			<span className='font-mono text-[12px] text-fontz/70'>{value}</span>
		</div>
	);
}
	...
	if (injuredOperators.length === 0) {
		return (
			<div className='flex items-center justify-center py-6'>
				<span className='font-mono text-[11px] tracking-widest text-lines/20 uppercase'>No Wounded</span>
			</div>
		);
	}
			...
			{injuredOperators.map((entry, index) => {
				...
				return (
					<div
						...
						{/* Avatar */}
						<div className='relative shrink-0'>
							<div className='w-9 h-9 rounded-full border border-amber-900/40 overflow-hidden bg-highlight'>
								<img
									className='w-full h-full object-cover object-top'
									src={entry.operator?.imageKey || entry.operator?.image || "/ghost/Default.png"}
									alt={entry.operator?.callSign || "Operator"}
							...
							<span className='absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border border-blk bg-amber-400' />
						</div>

						{/* Info */}
						<div className='flex-1 min-w-0 flex flex-col gap-1'>
							<div className='flex items-center justify-between gap-2'>
								<span className='font-mono text-[12px] text-fontz truncate'>
									{entry.operator?.callSign || "Unknown"}
								</span>
								<span className='font-mono text-[10px] tracking-widest uppercase text-amber-400 shrink-0'>
									WIA
								</span>
							</div>
							<div className='flex items-center gap-2'>
								<RecoveryBar progress={progress} />
								<span className='font-mono text-[9px] text-lines/30 tabular-nums shrink-0'>
						...
						{openSheet && (
							<FontAwesomeIcon icon={faChevronRight} className='text-[10px] text-lines/20 shrink-0' />
						)}
					</div>
				);
			})}
