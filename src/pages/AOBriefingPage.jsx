...
import { faChevronDown, faLocationDot, faSkullCrossbones, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
				...
				{threats.length === 0 ? (
					...
					<div className="flex flex-col gap-2.5">
						{threats.map((threat, i) => (
							<div key={i} className="border border-red-900/40 bg-red-950/10">
								{/* Threat header row */}
								<div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-red-900/30"
									style={{ background: "rgba(239,68,68,0.07)" }}>
									<div className="flex items-center gap-2 min-w-0">
										<FontAwesomeIcon
											icon={threat.unlockable ? faTriangleExclamation : faSkullCrossbones}
...
export default function AOBriefingPage() {
	...
	return (
		<div className="flex flex-col flex-1 min-h-0 overflow-hidden"
			...
			<div className="shrink-0 flex items-center justify-between px-4 py-1 border-b border-neutral-800/60"
			...
			<div className="shrink-0 flex items-center gap-3 px-4 py-2 border-b border-neutral-700/50 bg-neutral-950/80">
				<FontAwesomeIcon icon={weatherMeta.icon} className={`text-sm ${weatherMeta.color}`} />
				...
				<div className="relative flex items-center">
					...
					>
						{provinceKeys.map(k => (
							<option key={k} value={k}>{PROVINCE_DISPLAY_NAMES[k] || k}</option>
						))}
					</select>
					<FontAwesomeIcon icon={faChevronDown} className="absolute right-2 text-[8px] text-btn/50 pointer-events-none" />
				</div>
				...
				<div className="lg:w-[360px] xl:w-[400px] shrink-0 flex flex-col border-t lg:border-t-0 lg:border-l border-neutral-800/60 bg-neutral-950/70 lg:overflow-y-auto divide-y divide-neutral-800/50">
					...
					<div className="shrink-0 px-3 py-2.5 bg-neutral-900/50 border-b border-neutral-800/60">
						<div className="flex items-center justify-between gap-2">
							<div className="flex items-center gap-2 min-w-0">
								<FontAwesomeIcon icon={faLocationDot} className="text-[9px] text-btn/60 shrink-0" />
